...
x <- c(0:10) # no. of lags
par(mfrow=c(1,1))
plot(x,y_w, type="h", xlab="Lag", ylab="Theoretical WN ACF", ylim=c(-0.12,1.02)); abline(0,0)
acf(w, lag.max=10, ylab="Simulated WN ACF")
plot(x,y_v, type="h", xlab="Lag", ylab="Theoretical MA ACF", ylim=c(-0.12,1.02)); abline(0,0)
acf(v, lag.max=10, ylab="Simulated MA ACF")

# -----------------------------------------------------------------------------
# Figure 2.4: 10 and 50 day MA
...
# -----------------------------------------------------------------------------
# Figure 2.5: Actual and differenced stock price
...
tsplot(diff(Amazon), ylab="Differenced close price", col="red")

# -----------------------------------------------------------------------------
# Figure 2.6: ACF of differenced stock price
acf(diff(Amazon))

# -----------------------------------------------------------------------------
# Figure 2.7: AR(2) model with ACF and PACF
...
# -----------------------------------------------------------------------------
# Figure 2.8: AMZN price with regression line
...
# -----------------------------------------------------------------------------
# Figure 2.9: predicted 250 data points
par(mfrow=c(1,1))
tsplot(Amazon.full <- as.vector(AMZN$AMZN.Close[1260:2000]), col="red", ylim=c(min(Amazon.full), 800), ylab="Close Price")
...
fit4vals <- sapply(1:800, func)
lines(fit4vals, col="blue")
points(484, Amazon[484], col="black", pch=16)
abline(v=483, col="black")
text(x=375, y=400, "sample cut-off")
?text
legend('topleft', col=c("red", "blue"), lty=c(1,1), lwd = c(2,2), legend=c("Close price", "Model 4"), bty="n")
